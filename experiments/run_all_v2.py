"""
Fixed experiment runner: all bugs corrected, 5 seeds, proper evaluation.
"""
import sys, os
...
def build_and_train(A, gs, K, tr, va, device):
    ...
    # Tune lambda for classical methods via grid search
    best_lam, best_nmse = 0.1, float('inf')
    for lam_try in [0.005, 0.01, 0.02, 0.05, 0.1, 0.2, 0.5]:
        g = GroupFISTA(A, gs, lam=lam_try, max_iter=K)
        from src.utils.metrics import nmse_db as nmse_db_fn
        with torch.no_grad():
            xh = g.solve(va['y'][:200])
            v = nmse_db_fn(xh, va['x'][:200])
        if v < best_nmse:
            best_nmse = v
            best_lam = lam_try
    ...
    train_unfolded_model(lista, tr, va, num_epochs=TRAIN_EPOCHS,
                         lr=1e-4, batch_size=128, device=device)
    models['LISTA'] = lista
    ...
    for seed in range(N_SEEDS):
        ...
        key_models = {k: v for k, v in models.items()
                      if k in ['Group-ISTA','Group-FISTA','BlockLISTA',
                               'ALISTA','HyperLISTA','SH-block_soft','SH-hybrid']}
        ...
        for pn, pr in [('c1', np.linspace(0.001, 2.0, 12)),
                       ('c2', np.linspace(0.001, 1.5, 12)),
                       ('c3', np.linspace(0.1, 6.0, 12))]:
            sens = []
            for v in pr:
