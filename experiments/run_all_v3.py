"""
Upgraded experiment runner v3: includes Ada-BlockLISTA, reparameterized models, 5 seeds.
"""
import sys, os
...
N, M, GS, SG = 250, 125, 10, 5
K = 16
N_TRAIN, N_VAL, N_TEST = 5000, 1000, 2000
N_SEEDS = 5
N_TRIALS = 80
TRAIN_EPOCHS = 200


def build_and_train(A, gs, K, tr, va, device):
    models = {}
    ...
    for lam_try in [0.005, 0.01, 0.02, 0.05, 0.1, 0.2, 0.5]:
        ...
        with torch.no_grad():
            xh = g.solve(va['y'][:200])
            v = nmse_db(xh, va['x'][:200])
        if v < best_nmse:
            best_nmse = v
            best_lam = lam_try
    print(f"  Best lambda for ISTA/FISTA: {best_lam} (val={best_nmse:.2f} dB)")

    print("  [1] Group-ISTA")
    models['Group-ISTA'] = GroupISTA(A, gs, lam=best_lam, max_iter=K)
    print("  [2] Group-FISTA")
    ...
    print("  [6] Ada-BlockLISTA (tied)")
    abl_tied = AdaBlockLISTA(A, gs, num_layers=K, tied=True)
    train_unfolded_model(abl_tied, tr, va, num_epochs=TRAIN_EPOCHS * 2,
                         lr=5e-4, batch_size=128, progressive=False, device=device)
    models['Ada-BLISTA-T'] = abl_tied

    print("  [7] Ada-BlockLISTA (untied)")
    abl_untied = AdaBlockLISTA(A, gs, num_layers=K, tied=False)
    train_unfolded_model(abl_untied, tr, va, num_epochs=TRAIN_EPOCHS * 2,
                         lr=3e-4, batch_size=128, progressive=False, device=device)
    models['Ada-BLISTA-U'] = abl_untied
...
def run_all(device='cpu'):
    ...
    for seed in range(N_SEEDS):
        ...
        models = build_and_train(A, GS, K, tr, va, device)
        ...
        key_models = {k: v for k, v in models.items()
                      if k in ['Group-ISTA', 'Group-FISTA', 'BlockLISTA',
                               'ALISTA', 'HyperLISTA', 'Ada-BLISTA-T',
                               'SH-block_soft', 'SH-hybrid']}
        for sw, vals in mm_cfgs.items():
            print(f"  [MM] {sw}")
            for mn, mdl in key_models.items():
                r = mismatch_sweep(mdl, ds, sw, vals, GS, num_test=500, device=device)
        ...
        for c2v in [0.0, 0.01, 0.05, 0.1, 0.3, 0.5, 1.0]:
            ...
            with torch.no_grad():
                xh = m(te['y'].to(device))
                val = nmse_db(xh, te['x'].to(device))
            abl_all[f'mom_{c2v}']['nmse'].append(val if np.isfinite(val) else 10.0)
