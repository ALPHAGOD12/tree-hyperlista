...
Tree-CoSaMP & $-31.48 \pm 0.08$ & \textbf{1.000} & \textbf{1.000} & 0 \\
...
\textbf{TH-Hybrid (M3)} & $\mathbf{-28.66 \pm 0.10}$ & 0.663 & \textbf{1.000} & \textbf{3} \\
\bottomrule
\end{tabular}
\end{table}
...
Notably, Tree-CoSaMP achieves perfect precision and recall ($-31.5$ dB) as a classical method with exact support knowledge, providing an upper bound for model-based recovery. Tree-HyperLISTA closes the gap significantly while using only 3 learned parameters.
...
Tree-CoSaMP   & 14.79 & 0.537 & 27.04 & 0.839 & 28.67 & 0.927 \\
...
SH-topk (3p)      & 12.80 & 0.608 & 18.43 & 0.744 & 24.67 & 0.666 \\
\midrule
...
Table~\ref{tab:tree_imagecs} shows the image CS results. TH-hybrid (3 params) achieves the best PSNR/SSIM at both 25\% and 50\% CS ratios, reaching $31.66$ dB / 0.952 SSIM at 50\%---surpassing ALISTA (32 params, $35.18$ dB at 50\% but only $13.98$ dB at 10\%) in the structurally-constrained low-ratio regime. At 10\%, TH-hybrid ($13.10$ dB) matches or beats all other structured and elementwise methods except Tree-FISTA ($14.38$ dB), benefiting from the natural tree structure of wavelet coefficients. The block-aware SH-topk (3 params) underperforms TH-hybrid at all ratios, confirming that tree structure provides additional benefit over block structure for wavelet-domain signals.

\begin{figure}[t]
    \centering
    \includegraphics[width=0.95\textwidth]{tree_figures/tree_fig9_image_cs.pdf}
    \caption{Wavelet-domain image CS: PSNR (left) and SSIM (right) across CS ratios. TH-hybrid leads at 25\% and 50\%.}
...
\section{Layer Extrapolation}
\label{app:extrapolation}

Figure~\ref{fig:extrapolation} evaluates models trained with $K{=}16$ layers when evaluated with more layers ($K{=}16, 24, 32, 48, 64$). Tree-HyperLISTA degrades significantly beyond 24 layers (NMSE grows to $+107$ dB at $K{=}48$), while LISTA-Extra degrades gracefully. HyperLISTA and Tree-HyperLISTA-Elem also diverge. This motivates training with layer dropout or explicitly enforcing stability at extrapolated depths.

\begin{table}[h]
\centering
\caption{Layer extrapolation: NMSE (dB) at different test-time layer counts (trained at $K{=}16$).}
\label{tab:extrapolation}
\small
\begin{tabular}{lccccc}
\toprule
\textbf{Model} & $K{=}16$ & $K{=}24$ & $K{=}32$ & $K{=}48$ & $K{=}64$ \\
\midrule
LISTA-Extra          & $-13.28$ & $-13.96$ & $-14.46$ & $-15.18$ & $-15.65$ \\
HyperLISTA           & $-15.75$ & $-4.31$  & $+34.86$ & $+117.5$ & $+201.2$ \\
Tree-HyperLISTA-Elem & $-15.75$ & $-4.41$  & $+34.62$ & $+117.2$ & $+200.7$ \\
Tree-HyperLISTA      & $-28.66$ & $-20.90$ & $+8.70$  & $+107.1$ & $+224.3$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.75\textwidth]{tree_figures/tree_fig_extrapolation.pdf}
    \caption{Layer extrapolation: NMSE vs.\ test-time layers for models trained at $K{=}16$.}
...
Table~\ref{tab:tuning} compares three strategies for tuning the 3 hyperparameters: Backpropagation through unrolled iterations (TH-BP), grid search (TH-GS), and Bayesian optimization (TH-BO).

\begin{table}[h]
\centering
\caption{Comparison of tuning methods for Tree-HyperLISTA.}
\label{tab:tuning}
\begin{tabular}{lccc}
\toprule
\textbf{Method} & \textbf{NMSE (dB)} & \textbf{Tune Time (s)} & \textbf{\#Evals} \\
\midrule
TH-BP (backprop)          & $-0.64$ & 1.51 & 80 \\
TH-GS (grid search)       & $-7.14$ & 1.45 & 250 \\
TH-BO (Bayesian opt.)     & $-2.73$ & 1.29 & 5 \\
\bottomrule
\end{tabular}
\end{table}

Grid search achieves the best NMSE ($-7.14$ dB) with 250 evaluations. Bayesian optimization finds a reasonable solution ($-2.73$ dB) with only 5 evaluations. Backpropagation performs worst ($-0.64$ dB), likely because the non-differentiable tree projection creates poor gradients.

\begin{figure}[h]
    \centering
    \includegraphics[width=0.65\textwidth]{tree_figures/tree_fig_tuning.pdf}
    \caption{Tuning method comparison: NMSE achieved vs.\ tuning strategy.}
    \label{fig:tuning}
\end{figure}
...
\section{BSD500 Dictionary CS}
\label{app:bsd500}

We evaluate on a dictionary-based image CS task using BSD500 patches. At CS ratio 25\%, HyperLISTA achieves $18.06$ dB PSNR and TH-hybrid achieves $18.06$ dB PSNR---comparable performance, indicating that on natural image patches with learned dictionary atoms (which do not strictly follow tree-consistent sparsity), the tree-aware prior neither helps nor hurts relative to elementwise.

\section{Self-Supervised Image CS}
\label{app:ss_image_cs}

Table~\ref{tab:ss_imagecs} evaluates self-supervised variants of Tree-HyperLISTA that do not require ground-truth signal labels during inference adaptation. TH-SS ($N$-steps) performs test-time self-supervised gradient steps; TH-Amort ($N$-steps) uses an amortized network.

\begin{table}[h]
\centering
\caption{Self-supervised image CS: PSNR at three CS ratios. TH-Amort matches supervised TH-hybrid.}
\label{tab:ss_imagecs}
\small
\begin{tabular}{lcccc}
\toprule
\textbf{Method} & \textbf{Supervised?} & \textbf{CS 10\%} & \textbf{CS 25\%} & \textbf{CS 50\%} \\
\midrule
TH-hybrid (supervised)  & Yes & 13.10 & 18.82 & 30.96 \\
TH-SS-10steps           & No  & 11.71 & 13.53 & 20.85 \\
TH-SS-20steps           & No  & 11.71 & 13.53 & 20.85 \\
TH-Amort-5steps         & No  & 13.10 & 18.81 & 30.96 \\
TH-Amort-10steps        & No  & 13.09 & 18.82 & 30.96 \\
\bottomrule
\end{tabular}
\end{table}

TH-Amort closely matches the supervised TH-hybrid at all ratios, demonstrating that Tree-HyperLISTA can be effectively adapted without labeled data using amortized inference. The direct self-supervised gradient variant (TH-SS) underperforms significantly, especially at higher CS ratios.
