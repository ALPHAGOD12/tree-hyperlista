...
\textbf{Structured deep unfolding.} Recent work has explored block-aware unfolding (Ada-BlockLISTA \cite{fu2021adablocklista}, Struct-HyperLISTA), but tree-aware deep unfolding has not been explored. Our work is the first to combine tree-structured support selection with ultra-lightweight deep unfolding.
...
Table~\ref{tab:tree_imagecs} shows the image CS results. TH-hybrid (3 params) achieves the \textbf{best SSIM at all three CS ratios} (0.707, 0.834, 0.952) and the best PSNR among all lightweight structured methods ($\leq$3 params). At 50\%, TH-hybrid reaches $31.66$ dB, surpassing all classical and structured baselines and approaching ALISTA (35.18 dB, 32 params) while using $10\times$ fewer parameters. The block-aware SH-topk (3 params) underperforms TH-hybrid at every ratio, directly confirming that tree structure provides additional discriminative benefit over block structure for wavelet-domain signals where parent-child coefficient dependencies are meaningful.
